...
def encode_queries_pca(pca_model, query_embeds, batch_size, dev_queries):
    qembeds = []
    qembeds_raw = []
    qids = []
    for qid, embedding in query_embeds.items():
...
def encode_dos_pca(pca_model, doc_embeds, batch_size):
    dembeds = []
    dids = []
    doc_embeds = list(doc_embeds.items())
    for start in tqdm(range(0, len(doc_embeds), batch_size)):
...
def trans_queries(query_embeds, dev_queries):
    qembeds = []
    qids = []
    for qid, embedding in query_embeds.items():
        if qid in dev_queries:
            qids.append(qid)
            qembeds.append(embedding)
    return qids, qembeds


def trans_dos(doc_embeds):
    dids = list(doc_embeds.keys())
    dembeds = list(doc_embeds.values())
    return dids, dembeds
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', type=str, default=None, required=True, help='Checkpoint path or PCA model path.')
    parser.add_argument('--model', type=str, required=True, help='Model name like ConAE, KL, CE, PCA.')
    ...
    if args.model.lower() == 'conae' or args.model.lower() == 'kl':
        ...
        logger.info('encoding queries...')
        qids, qembeds = encode_queries_pca(pca_model, query_embeds, args.batch_size, dev_queries)
        logger.info('encoding docs...')
        dids, dembeds = encode_dos_pca(pca_model, doc_embeds, args.batch_size)
    else:
        logger.info('encoding queries...')
        qids, qembeds = trans_queries(query_embeds, dev_queries)
        logger.info('encoding docs...')
        dids, dembeds = trans_dos(doc_embeds)
    ...
    topN = 1000
    if args.hnsw:
        dembeds = [(step, dembed) for step, dembed in enumerate(dembeds)]
        index = DenseHNSWFlatIndexer()
        index.init_index(args.output_dim)
        index.index_data(dembeds)
        index.serialize("hnsw_{0}".format(args.output_dim))
        logger.info('Testing latency...')
        start = time.perf_counter()
        results = index.search_knn(np.array(qembeds), topN)
        end = time.perf_counter() - start
        dev_I = [result[0] for result in results]
        logger.info("total time " + str(end))
        logger.info("time every query " + str(end / len(dev_I)))
    else:
